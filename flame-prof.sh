...
function usage {
    echo "usage: flame-perf COMMAND ARGS..."
    echo "       flame-perf -p PID"
}
...
if ! which perf > /dev/null 2>&1; then
    echo "can't find 'perf'"
    exit 1
fi

if [ "$MODE" = "cmd" ]; then
    perf record -o /tmp/flame_prof_data_$$ -F 99 -a -g -- $@
else
    perf record -o /tmp/flame_prof_data_$$ -F 99 -a -g -p $PID
fi

perf script -i /tmp/flame_prof_data_$$ > /tmp/flame_prof_stacks_$$
...
_HERE_
) | perl -w -- - /tmp/flame_prof_stacks_$$ > /tmp/flame_prof_folded_$$
...
_HERE_
) | perl -w -- - /tmp/flame_prof_folded_$$

rm -f /tmp/flame_prof_*_$$
